...
export default function PolaroidExecutiveUnifiedDemo() {
  ...
  const lastPurchaseDate = "24/12/2025";
  ...
  const triggerConnect = () => {
    ...
    setTimeout(() => {
      setStep(3);
      setClv(378.98); 
      addEvent({ tealium_event: "visitor_identity_resolved", uid: "u_123456", clv: 378.98 });
      setShowNudge(true);
    }, 1500);
  };
  ...
  return (
    <main className="fixed inset-0 bg-[#051838] text-white p-4 font-sans flex flex-col overflow-hidden text-left">
      <header className="w-full max-w-7xl mx-auto mb-6 shrink-0 border-b border-[#68D8D5]/30">
        <h1 className="text-xl font-black italic tracking-tighter text-[#68D8D5] pb-2">
            ...
            <AnimatePresence>
              ...
              {showFilmNudge && (
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="absolute top-1/2 left-4 right-4 -translate-y-1/2 bg-white text-black p-6 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] z-[60] border-2 border-black">
                   ...
                   <p className="text-[11px] text-slate-600 mt-2 mb-4 font-medium leading-snug">Keep the creative juices flowing. You're down to your last shot.</p>
              ...
              {orderComplete && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-[70] bg-[#68D8D5] flex flex-col items-center justify-center p-8 text-[#051838]">
                  ...
                  <p className="text-2xl font-black uppercase italic tracking-tighter">Order Success</p>
                  <p className="text-xs font-bold mt-2">Film Bundle Sent to demotealium@gmail.com</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
            ...
            <div className="p-4 space-y-4 text-[10px] overflow-y-auto scrollbar-thin">
              ...
              <section>
                ...
                <div className="space-y-1">
                  ...
                  <Attribute label="Videos Watched" value={(isPlayingVideo || step >= 6) ? "Lab Guide, Now+ Creative Tips" : (step >= 3 ? "Lab Guide" : "---")} />
                </div>
              </section>

              <section>
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-2 border-b border-slate-100 pb-1 text-left text-left">Badges</p>
              ...
              <section className="pb-2">
                ...
                <div className="space-y-1.5 text-left text-left">
                  ...
                  {(showFilmNudge || orderComplete) && <div className="p-2 bg-[#051838] text-white rounded font-bold text-[8px] flex items-center justify-between border-l-2 border-[#68D8D5] shadow-lg shadow-black/30"><span className="flex items-center gap-2 text-left text-left text-left"><Trophy size={10} className="text-[#68D8D5]"/> Push: Film Replenishment</span><span className="text-[#68D8D5] animate-pulse">SENT</span></div>}
                </div>
              </section>
            </div>
          </div>
...
function Attribute({ label, value, color }: { label: string, value: string, color?: string }) {
  return (
    <div className="flex justify-between items-center py-0.5 border-b border-slate-50 text-left">
      ...
      <span className={`text-[9px] font-black tracking-tight text-right ${color || 'text-slate-800'}`}>{value}</span>
    </div>
  );
}
...
function Badge({ label, on }: { label: string, on: boolean }) {
  return (
    <div className={`p-1.5 rounded border text-[6px] font-black uppercase flex items-center gap-1.5 transition-all duration-700 ${on ? 'bg-green-100 border-green-400 text-green-800 shadow-md' : 'bg-slate-50 border-slate-200 text-slate-500 opacity-80'}`}>
