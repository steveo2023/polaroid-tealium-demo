    ...
    <main className="fixed inset-0 bg-[#051838] text-white p-4 font-sans flex flex-col overflow-hidden text-left">
      ...
      <div className="max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
        ...
        <div className="flex flex-col h-full min-h-0">
          ...
          <div className="relative flex-1 bg-black rounded-[2.8rem] border-[7px] border-slate-800 shadow-2xl overflow-hidden shadow-cyan-900/20">
        ...
        {/* PANE 2: EVENTSTREAM */}
        <div className="flex flex-col h-full min-h-0">
          <h2 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2 italic tracking-tighter">2. Tealium EventStream</h2>
          <div className="flex-1 bg-[#030d1f] rounded-xl border border-slate-700 p-4 font-mono text-[10px] overflow-y-auto scrollbar-thin">
            {events.map((ev, i) => (
        ...
        <div className="flex flex-col h-full min-h-0">
          ...
          <div className="flex-1 bg-white rounded-xl text-slate-900 flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="p-3 space-y-3 overflow-y-auto scrollbar-thin">
              ...
              <section className="bg-slate-50/50 p-2 rounded-lg border border-slate-100">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center py-1">
                    ...
                    <span className={`font-black transition-all duration-700 ${orderComplete ? 'text-2xl text-green-600' : 'text-xs text-[#051838]'}`}>£{clv.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center py-1 border-b border-slate-100 pb-2">
                    ...
                    <span className={`font-black transition-all duration-700 ${orderComplete ? 'text-2xl text-green-600' : 'text-xs text-slate-400'}`}>{lastOrderValue > 0 ? `£${lastOrderValue.toFixed(2)}` : "£0.00"}</span>
                  </div>
              ...
              <section>
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-2 border-b pb-1">Badges</p>
              ...
              <section>
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-1">Audiences</p>
                <div className="space-y-1">
              ...
              <section className="pb-4">
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-1">Activations</p>
                <div className="space-y-1">
                  {step >= 3 && <div className="p-1.5 bg-slate-900 text-white rounded font-bold text-[8px] flex justify-between border-l-2 border-[#68D8D5]"><span><Send size={10} className="inline mr-1 text-[#68D8D5]"/> Onboarding</span><CheckCircle size={10} className="text-green-500"/></div>}
                  {manualModeActive && <div className="p-1.5 bg-slate-900 text-white rounded font-bold text-[8px] flex justify-between border-l-2 border-[#68D8D5]"><span><Zap size={10} className="inline mr-1 text-[#68D8D5]"/> Night Mode</span><span className="text-green-500">ACTIVE</span></div>}
                  {(showFilmNudge || orderComplete) && <div className="p-1.5 bg-[#051838] text-white rounded font-bold text-[8px] flex justify-between border-l-2 border-[#68D8D5]"><span><Trophy size={10} className="inline mr-1 text-[#68D8D5]"/> Replenishment</span><span className="text-[#68D8D5] animate-pulse">SENT</span></div>}
                </div>
              </section>
            </div>
          </div>
...
function Badge({ label, on, customColor }: { label: string, on?: boolean, customColor?: string }) {
  return (
    <div className={`p-1 rounded border text-[6px] font-black uppercase flex items-center gap-1 ${on ? 'bg-green-100 border-green-400 text-green-800' : (customColor || 'bg-slate-50 border-slate-100 text-slate-400 opacity-80')}`}>
