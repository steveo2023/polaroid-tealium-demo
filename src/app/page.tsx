...
export default function PolaroidUltimateROIPitch() {
  ...
  // Calculate 160 days ago on initialization
  useEffect(() => {
    const date = new Date();
          ...
          <div className="flex-1 bg-white rounded-xl text-slate-900 flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="p-4 space-y-4 text-[10px] overflow-y-auto scrollbar-thin">
              ...
              <section className="pb-2">
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-2 border-b border-slate-100 pb-1 text-left">Activations</p>
                <div className="space-y-1.5 text-left">
                  {step >= 3 && <Activation label="Push: Manual Onboarding" icon={<Send size={10} className="text-[#68D8D5]"/>} active={true}/>}
                  {manualModeActive && <Activation label="Push: Night Mode Manual" icon={<Zap size={10} className="text-[#68D8D5]"/>} active={true}/>}
                  {(showFilmNudge || orderComplete) && <Activation label="Push: Film Replenishment" icon={<Trophy size={10} className="text-[#68D8D5]"/>} active={true} pulse={!orderComplete}/>}
                </div>
              </section>
            </div>
          </div>
...
function Attribute({ label, value }: { label: string, value: string }) {
  return (
    <div className="flex justify-between items-center py-0.5 border-b border-slate-50 text-left">
      ...
      <span className="text-[9px] font-black tracking-tight text-slate-800">{value}</span>
    </div>
  );
}
...
function Activation({ label, icon, active, pulse }: { label: string, icon: any, active: boolean, pulse?: boolean }) {
  return (
    <div className={`p-2 bg-slate-900 text-white rounded font-bold text-[8px] flex items-center justify-between border-l-2 border-[#68D8D5]`}>
      <span className="flex items-center gap-2">{icon} {label}</span>
      <span className={pulse ? "animate-pulse text-[#68D8D5]" : "text-green-500"}>{pulse ? "SENT" : "ACTIVE"}</span>
    </div>
  );
}