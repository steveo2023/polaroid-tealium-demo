  ...
  return (
    <main className="fixed inset-0 bg-[#051838] text-white p-4 font-sans flex flex-col overflow-hidden text-left">
      <header className="w-full max-w-7xl mx-auto mb-2 shrink-0 border-b border-[#68D8D5]/30">
        <h1 className="text-xl font-black italic tracking-tighter text-[#68D8D5] pb-1 text-left">
      ...
      <div className="max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 min-h-0 text-left">
        ...
        <div className="flex flex-col h-full min-h-0">
          ...
          <div className="relative flex-1 bg-black rounded-[2.5rem] border-[6px] border-slate-800 shadow-2xl overflow-hidden">
            ...
            <AnimatePresence>
              {showNudge && (
                <motion.div initial={{ y: -50 }} animate={{ y: 20 }} className="absolute top-10 left-4 right-4 bg-[#051838] text-white p-4 rounded-2xl border-l-4 border-[#68D8D5] z-50 shadow-2xl">
                   <p className="font-bold text-xs italic uppercase tracking-tighter text-[#68D8D5]">Master Your Now+</p>
                   <p className="text-[10px] text-slate-200 mt-1 leading-relaxed font-medium text-left">Master Expert creative techniques with our Tutorial.</p>
              ...
              {showApertureNudge && (
                <motion.div initial={{ y: 50 }} animate={{ y: -100 }} className="absolute bottom-16 left-4 right-4 bg-[#051838] text-white p-5 rounded-2xl border-t-4 border-[#68D8D5] z-50 shadow-2xl">
                   <p className="font-bold text-xs italic uppercase tracking-tighter text-[#68D8D5]">Evening Capture Detected</p>
                   <p className="text-[10px] text-slate-200 mt-1 italic leading-relaxed text-left">We've detected that your shooting at night. Why not try Manual Mode to get more control over your photos.</p>
                   <button onClick={handleActivateManual} className="w-full mt-3 py-2.5 bg-[#68D8D5] text-[#051838] text-[10px] font-black rounded-lg uppercase shadow-xl">Activate Manual Mode</button>
                </motion.div>
              )}
              {showFilmNudge && (
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="absolute top-1/2 left-4 right-4 -translate-y-1/2 bg-white text-black p-5 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] z-[60] border-2 border-black">
                   <Trophy className="mx-auto mb-1 text-[#68D8D5]" size={28} />
                   <p className="font-black text-xs uppercase italic text-center">Creative Flow Active</p>
                   <p className="text-[10px] text-slate-600 mt-1 mb-3 font-medium leading-snug text-center">Keep the creative juices flowing. You're down to your last few shot. Only 2 pictures left on the film.</p>
                   <button onClick={handleOrderFilm} className="w-full py-2.5 bg-[#68D8D5] text-[#051838] text-[10px] font-black rounded-lg uppercase flex items-center justify-center gap-2 shadow-xl"><ShoppingCart size={12} /> Order New Film Now</button>
                </motion.div>
              )}
              {orderComplete && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-[70] bg-[#68D8D5] flex flex-col items-center justify-center p-8 text-[#051838]">
                  ...
                  <p className="text-xl font-black uppercase italic tracking-tighter text-center">Order Success</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          <div className="flex-1 bg-[#030d1f] rounded-xl border border-slate-700 p-3 font-mono text-[9px] overflow-y-auto scrollbar-thin">
            {events.map((ev, i) => (
              <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-3 p-2 rounded border border-slate-800 bg-blue-500/5 text-left">
                <p className="text-[#68D8D5] font-bold mb-1 uppercase tracking-tighter text-left">[{ev.timestamp}] Event Streamed</p>
        ...
        <div className="flex flex-col h-full min-h-0 text-left">
          ...
          <div className="flex-1 bg-white rounded-xl text-slate-900 flex flex-col shadow-2xl overflow-hidden">
            <div className="bg-[#68D8D5] p-2 flex justify-between items-center border-b border-black/10 shrink-0 text-left">
               <div className="flex items-center gap-2 text-[#051838] font-black uppercase italic text-xs text-left"><User size={14}/> Unified Profile</div>
               {step >= 3 && <div className="text-[8px] font-black px-1.5 py-0.5 bg-[#051838] text-white rounded">ID: u_123456</div>}
            </div>

            <div className="p-2 space-y-1.5 overflow-y-hidden text-left">
              <section className="bg-slate-50/50 p-2 rounded-lg border border-slate-100 text-left">
                <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest mb-1 border-b border-slate-200 pb-0.5 text-left">Unified Profile Data</p>
                <div className="space-y-0.5">
                  ...
                  <div className="flex justify-between items-center border-b border-slate-100 pb-0.5 text-left">
              ...
              <section className="text-left">
                ...
                <div className="grid grid-cols-3 gap-1">
                  ...
                  <Badge label="Aperture" on={videoTime >= 31} />
                  <Badge label="Exposure" on={videoTime >= 45} />
                  <Badge label="Painting" on={videoTime >= 67} />
                  <Badge label="Tripod" on={videoTime >= 80} />
                  <Badge label="Expert" on={step >= 4} />
                  <Badge label="Film Sub" on={false} />
                </div>
              </section>
              ...
              <section className="text-left">
                ...
                <div className="space-y-0.5 text-left">
                  <div className={`p-1 rounded font-black uppercase text-[7px] border ${step >= 3 ? 'bg-[#007CC2] text-white border-[#006699]' : 'bg-slate-50 text-slate-300 opacity-60'}`}>New Owner - Auto Only</div>
                  <div className={`p-1 rounded font-black uppercase text-[7px] border ${showFilmNudge || orderComplete ? 'bg-[#007CC2] text-white border-[#006699]' : 'bg-slate-50 text-slate-300 opacity-60'}`}>Film Replenishment Needed</div>
                </div>
              </section>

              <section className="pb-1 text-left">
                <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest mb-0.5 text-left text-left">Activations</p>
                <div className="space-y-0.5 text-left">
                  {step >= 3 && <div className="p-1 bg-slate-900 text-white rounded font-bold text-[7px] flex justify-between border-l-2 border-[#68D8D5] text-left"><span><Send size={8} className="inline mr-1 text-[#68D8D5]"/> Onboarding</span><CheckCircle size={8} className="text-green-500"/></div>}
                  {manualModeActive && <div className="p-1 bg-slate-900 text-white rounded font-bold text-[7px] flex justify-between border-l-2 border-[#68D8D5] text-left"><span><Zap size={8} className="inline mr-1 text-[#68D8D5]"/> Night Mode</span><span className="text-green-500">ACTIVE</span></div>}
                  {(showFilmNudge || orderComplete) && <div className="p-1 bg-[#051838] text-white rounded font-bold text-[7px] flex justify-between border-l-2 border-[#68D8D5] text-left"><span><Trophy size={8} className="inline mr-1 text-[#68D8D5]"/> Replenishment</span><span className="text-[#68D8D5] animate-pulse">SENT</span></div>}
                </div>
              </section>
            </div>
          </div>
...
function Badge({ label, on }: { label: string, on?: boolean }) {
  return (
    <div className={`p-0.5 rounded border text-[5.5px] font-black uppercase flex items-center justify-center gap-0.5 transition-all duration-700 ${on ? 'bg-green-100 border-green-400 text-green-800' : 'bg-red-50 border-red-300 text-red-500 opacity-90'}`}>
      <CheckCircle size={6} className={on ? 'text-green-600' : 'text-red-300'} /> {label}
    </div>
  );
}